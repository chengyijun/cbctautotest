...
@file: CBCT Auto Test 之DX侧位自动化拍摄流程脚本
...
from dx_helper import select_patient, select_body_type, select_electricity_min
from utils import click_action, click_double_action, wait_until_checkpoint_appear, select_app, add_patient, \
    wait_before_qrps, wait_before_ps_ui_load, wait_before_rebuild, get_pos
...
class DXAutoTest:
    ...
    def image_collect(self):
        ...
        wait_before_qrps()
        x, y = get_pos('finish_take_shot')
        click_action(x, y, wait=1)
        wait_before_rebuild()
        print("拍片完成")
