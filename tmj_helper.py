...
def select_teeth_shape() -> None:
    ...
    # print(choice)
    if choice == 0:
        click_action(201, 629, duration=0.5, wait=0.5)
    elif choice == 1:
        click_action(289, 629, duration=0.5, wait=0.5)
    else:
        click_action(374, 629, duration=0.5, wait=0.5)
...
def select_electricity() -> None:
    ...
    for _ in range(u):
        click_action(198, 707)
    il = random.randint(0, 3)
    for _ in range(il):
        click_action(198, 747)
    ir = random.randint(0, 5)
    for _ in range(ir):
        click_action(374, 747)
